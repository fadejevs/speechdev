...
export const useAzureSpeechRecognition = (
  ...
  const handleNewTranscription = useCallback((data) => {
  ...
  const createStartRecognizer = useCallback((socketRef) => {
    const startRecognizer = (deviceId = null) => {
      ...
      let processingState = 'idle'; // Track if we're currently processing
              ...
              interimFallbackTimeout = setTimeout(() => {
                ...
                if (lastInterimText === text && lastInterimText.trim().length > 0) {
                  ...
                  setTimeout(() => {
                    ...
                    setTimeout(() => { processingState = 'idle'; }, 100); // Reset state after processing
                  }, 0);
